...
if ( not -e "$RDIR/train.lc.cor" ) {
    ...
    foreach my $MORE (@MORE_RELEASE) {
        message("Adding more training data: $MORE");
        die if ( not -s $MORE );
        `cat $MORE | cut -f 1 >> $RDIR/train.lc.err`;
        `cat $MORE | cut -f 2 >> $RDIR/train.lc.cor`;
	`cat $MORE | cut -f 1 >> $RDIR/train.err`;
	`cat $MORE | cut -f 2 >> $RDIR/train.cor`;
